...
describe('recommend', () => {
  it('posts a query and filters to the same-origin endpoint', async () => {
    ...
    const result = await recommend('iphone', filters, undefined, fetcher);
  ...
  it('uses the API error message when one is available', async () => {
    ...
    await expect(recommend('iphone', { brands: [], colors: [], price: null }, undefined, fetcher)).rejects.toThrow(
  ...
  it('handles non-JSON errors and malformed success responses', async () => {
    ...
    await expect(recommend('iphone', { brands: [], colors: [], price: null }, undefined, failed)).rejects.toThrow(
      'Search failed (502)'
    );
    await expect(recommend('iphone', { brands: [], colors: [], price: null }, undefined, malformed)).rejects.toThrow(
