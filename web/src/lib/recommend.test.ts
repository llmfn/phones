...
describe('recommend', () => {
  it('posts a query and empty filters to the same-origin endpoint', async () => {
    ...
    const result = await recommend('iphone', { fetcher });
  ...
  it('uses the API error message when one is available', async () => {
    ...
    await expect(recommend('iphone', { fetcher })).rejects.toThrow('Search is unavailable');
  });
  ...
  it('handles non-JSON errors and malformed success responses', async () => {
    ...
    await expect(recommend('iphone', { fetcher: failed })).rejects.toThrow('Search failed (502)');
    await expect(recommend('iphone', { fetcher: malformed })).rejects.toThrow('Search returned an invalid response');
  });
});
